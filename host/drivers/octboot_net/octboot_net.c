...
static unsigned int device_id_loki = 0xb400;
static unsigned int device_id_thor = 0xbc00;
...
static void octboot_net_init_work(struct work_struct *work)
{
	...
	while ((octnet_pci_dev = pci_get_device(vendor_id, device_id_loki,
					octnet_pci_dev))) {
		if (octnet_pci_dev == NULL) {
			pr_err("Invalid PCI bus or slot number\n");
			return;
		}
		pr_err("Found Loki Device\n");
		octnet_pci_dev_arr[octnet_num_device++] = octnet_pci_dev;
	}
	octnet_pci_dev = NULL;
	while ((octnet_pci_dev = pci_get_device(vendor_id, device_id_thor,
					octnet_pci_dev))) {
		if (octnet_pci_dev == NULL) {
			pr_err("Invalid PCI bus or slot number\n");
			return;
		}
		pr_err("Found Thor Device\n");
		octnet_pci_dev_arr[octnet_num_device++] = octnet_pci_dev;
	}
