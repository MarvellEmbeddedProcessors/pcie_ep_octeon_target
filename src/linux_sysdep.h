...
#define CAV_NET_SKB_PAD 	NET_SKB_PAD

#define RCV_BUF_SIZE  (SKB_DATA_ALIGN(CN83XX_OQ_BUF_SIZE + NET_SKB_PAD) + \
                       SKB_DATA_ALIGN(sizeof(struct skb_shared_info)))

#define RCV_BUF_MAP_SIZE  (RCV_BUF_SIZE - CAV_NET_SKB_PAD)

#define cav_net_kmalloc(size, flags)	kmalloc(size, flags)
#define cav_net_kfree(ptr)				kfree(ptr)

/* Function used to get skb user count */
...
static inline uint8_t cav_net_get_skb_data_ref_count(void *buffer)
{
	struct sk_buff *skb = buffer;
	return (uint8_t) atomic_read(&skb_shinfo(skb)->dataref) &
	    SKB_DATAREF_MASK;
}

static inline void cav_net_make_skb_head_null(void *buffer)
{
	struct sk_buff *skb = buffer;
	/* This will avoid data buffer free */
	skb->head = NULL;
}

static inline void cav_net_free_skb(void *buffer)
{
	kfree_skb((struct sk_buff *)buffer);
}

static inline cavium_netbuf_t *cav_net_build_skb(void *data,
						 unsigned int frag_size)
{
	return build_skb(data, frag_size);
}

static inline void cav_net_skb_reserve(cavium_netbuf_t * skbptr, int len)
{
	return skb_reserve(skbptr, len);
}

static inline unsigned char *cav_net_skb_put(cavium_netbuf_t * skbptr,
					     unsigned int len)
{
	return skb_put(skbptr, len);
}

static inline cavium_netbuf_t *cav_net_skb_get(cavium_netbuf_t * skbptr)
{
...
/* $Id: linux_sysdep.h 163569 2017-07-25 15:58:46Z mchalla $ */
