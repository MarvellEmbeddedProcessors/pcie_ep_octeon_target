...
int octeon_tx_enable_msix_interrupts(octeon_device_t * oct)
{
	...
	ret = pci_alloc_irq_vectors(oct->pci_dev, oct->num_irqs, oct->num_irqs,
				    PCI_IRQ_MSIX);
	if(ret) {
...
#endif
{
	...
	if(oct_dev->octeon_id) {
		cavium_print_msg("Skipping device:%d\n", oct_dev->octeon_id);
		return 0;
	}
#endif
...
#if 0 //TODO: enable when IOQ support is added
...
#ifdef USE_SINGLE_PF
	/* Nothing to do for uninitialized devices, just return */
	if(oct_dev->octeon_id)
		goto free_mem;
#endif
...
#if 0 //TODO: enable while adding 93xx/96xx support
...
int octeon_device_init(octeon_device_t * octeon_dev)
{
...
#if 0 //TODO: may need to enable when IOQs are suported
	...
	if(octeon_dev->chip_id == OCTEON_CN83XX_PF ||
		...
		poll_ops.fn_arg = 0UL;
		poll_ops.ticks = 5*CAVIUM_TICKS_PER_SEC;
		strcpy(poll_ops.name, "NPU Handshake Thread");
		octeon_register_poll_fn(octeon_dev->octeon_id, &poll_ops);
	}else {
		/* Register a Host - Firmware (OCTEON) handshake poll function */
...
/* $Id: octeon_main.c 173626 2019-03-28 09:06:45Z snilla $ */
