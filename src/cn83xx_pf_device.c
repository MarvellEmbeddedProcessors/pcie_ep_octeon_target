...
static void cn83xx_setup_iq_regs(octeon_device_t * oct, int iq_no)
{
	...
	reg_val =
	    octeon_read_csr64(oct,
			      CN83XX_SDP_EPF_R_IN_INT_LEVELS(oct->epf_num,
							     iq_no));
...
cvm_intr_return_t cn83xx_interrupt_handler(void *dev)
{
	...
	if (reg_val) {
		cavium_print_msg("received DMA_RINT intr: 0x%016llx\n",
				 reg_val);
		octeon_write_csr64(oct, CN83XX_SLI_EPF_DMA_RINT(oct->epf_num),
				   reg_val);
//TODO: this is the interrupt for all notifications from NPU to host for RPC/mgmt-netdev/npu-agent/eth_port-mgmt
		goto irq_handled;
	}
...
static void cn83xx_get_pcie_qlmport(octeon_device_t * oct)
{
...
static void cn83xx_setup_reg_address(octeon_device_t * oct)
{
