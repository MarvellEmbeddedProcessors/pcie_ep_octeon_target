...
#if 0 //TODO: enable when 93xx is supported
...
int oct_init_base_module(int octeon_id, void *octeon_dev)
{
	...
	/*Wait for some time(till cores reach data loop) befores sending start command */
...
octeon_wait_for_npu_base(void *octptr, unsigned long arg UNUSED)
{
	...
	if(octeon_dev->chip_id == OCTEON_CN93XX_PF) {
		printk("%s: CN93xx chip is not supported\n", __func__);
		return OCT_POLL_FN_CONTINUE;
	} else {
...
#if 0 //TODO: not required; remove it
...
/* $Id: octeon_device.c 173587 2019-03-19 10:40:44Z snilla $ */
