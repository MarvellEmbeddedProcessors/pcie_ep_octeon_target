...
static struct seq_operations init_ioqs_seq_ops = {
	.start = proc_seq_start,
	.next = proc_seq_next,
	.stop = proc_seq_stop
};

static int init_ioqs_open(struct inode *inode, struct file *file)
{
	int ret = seq_open(file, &init_ioqs_seq_ops);
	if (!ret) {
		struct seq_file *seq = file->private_data;
...
static ssize_t
init_ioqs_write(struct file *file, const char __user * buffer, size_t count,
	  loff_t * offp)
{
	char str[16];
	count = count > sizeof(str) ? sizeof(str) : count;
	...
	if(str[0] == '1') {
		/* Start base ioq initialization */
		octeon_register_base_handler();
	} else {
		/* Writing other than 1 will not have any effect */
	}
...
#ifdef BUILD_FOR_EMULATOR
struct file_operations proc_init_ioqs_fops = {
open:	init_ioqs_open,
write:	init_ioqs_write
};
#endif
...
#ifdef BUILD_FOR_EMULATOR
	node =
	    cavium_proc_create_data("init_ioqs", 0444, root, &proc_init_ioqs_fops,
				    octeon_dev);
	if (node == NULL)
		goto proc_fail;
	SET_PROC_OWNER(node);
#if LINUX_VERSION_CODE < KERNEL_VERSION(2,6,26)
	node->proc_fops = &proc_init_ioqs_fops;
	node->data = octeon_dev;
#endif
...
/* $Id: cavium_proc.c 171042 2018-04-11 11:59:28Z snilla $ */
