...
/* cnxk/otx2 does not support RTE_PTYPE_L2_ETHER.
 * so just assume l2 is ether for unknown (0)
 */
static const int l2_lens_arr[MAX_L2_TYPES] = { [L2FWD_PTYPE_UNKNOWN]
					       = RTE_ETHER_HDR_LEN,
					       [L2FWD_PTYPE_L2_ETHER]
					       = RTE_ETHER_HDR_LEN,
					       [L2FWD_PTYPE_L2_ETHER_VLAN]
					       = L2FWD_ETHER_VLAN_HDR_LEN };

static const int l3_lens_arr[MAX_L3_TYPES] = { [L2FWD_PTYPE_L3_IPV4]
					       = sizeof(struct rte_ipv4_hdr),
					       [L2FWD_PTYPE_L3_IPV6]
					       = sizeof(struct rte_ipv6_hdr) };

static const uint64_t l3_ol_flags[MAX_L3_TYPES]  = { [L2FWD_PTYPE_L3_IPV4]
						     = (L2FWD_PCIE_EP_TX_IPV4 |
							L2FWD_PCIE_EP_TX_IP_CKSUM),
						     [L2FWD_PTYPE_L3_IPV6]
						     = L2FWD_PCIE_EP_TX_IPV6 };

static const uint64_t l4_ol_flags[MAX_L4_TYPES]  = { [L2FWD_PTYPE_L4_TCP]
						     = L2FWD_PCIE_EP_TX_TCP_CKSUM,
						     [L2FWD_PTYPE_L4_UDP]
						     = L2FWD_PCIE_EP_TX_UDP_CKSUM };
/*
...
static inline void
drop_pkt(uint32_t portid, struct rte_mbuf *m, unsigned int queue)
{
...
static void
l2fwd_simple_forward(struct rte_mbuf *m, unsigned int portid, unsigned int tx_q)
{
	unsigned int dst_port = RTE_MAX_ETHPORTS;
	unsigned int cksum_offload = 0;
...
static int
l2fwd_launch_one_lcore(__always_unused void *dummy)
{
	l2fwd_main_loop();
	return 0;
}
